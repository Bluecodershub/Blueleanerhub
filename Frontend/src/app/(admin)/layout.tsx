...
import {
  ...
  GraduationCap, ClipboardCheck, FileCheck2, CreditCard, ListChecks,
} from 'lucide-react'
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <aside className="sticky top-0 z-40 hidden h-screen w-64 flex-col border-r border-border bg-card lg:flex">
        <div className="flex h-16 items-center gap-3 px-6 border-b border-border">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-rose-600 to-red-500 shadow-lg shadow-rose-600/25">
            <Shield className="h-5 w-5 text-white" />
          </div>
          <div>
            <p className="font-bold text-sm text-foreground leading-none">Admin Panel</p>
            <p className="text-[10px] text-rose-400 font-mono uppercase tracking-widest mt-0.5">Bluelearnerhub</p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={cn('h-5 w-5', isActive ? 'text-white' : 'text-rose-500/70')} />
                <span className="truncate">{item.title}</span>
                {isActive && (
                  <motion.div layoutId="admin-active-pill" className="absolute left-0 h-5 w-1 rounded-full bg-white" />
                )}
              </Link>
            )
          })}
        ...
        <div className="border-t border-border/50 p-4 space-y-2">
          <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-secondary/30 p-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-rose-600/20 font-bold text-rose-400 text-sm">
              {user?.fullName?.[0]?.toUpperCase() ?? 'A'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-foreground">{user?.fullName ?? 'Admin'}</p>
              <p className="text-[10px] text-rose-400 font-mono">Administrator</p>
            </div>
          </div>
          <button
            onClick={logout}
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}
                      className={cn(
                        ...
                        isActive ? 'bg-rose-600 text-white' : 'text-muted-foreground hover:bg-secondary/50'
                      )}
                    >
