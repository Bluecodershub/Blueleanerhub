  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto bg-white/40 p-4 dark:bg-transparent">
      ...
      {loading ? (
        ...
        <div className="px-2 py-8 text-center text-xs text-gray-400 dark:text-muted-foreground">
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <button
              ...
              className="w-full rounded-xl border border-sky-100/80 bg-white/90 p-3 text-left shadow-sm backdrop-blur transition-colors hover:border-sky-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/60 dark:border-border dark:bg-card/90 dark:hover:border-sky-600"
            >
              <div className="mb-2 flex items-center gap-2 text-[11px] uppercase tracking-wide text-gray-400">
                <Quote className="h-3.5 w-3.5" />
                {item.sourceTitle}
              </div>
              <div className="line-clamp-4 whitespace-pre-wrap text-sm text-gray-800 dark:text-foreground">
                {item.quote}
              </div>
              {item.note ? (
                <div className="mt-2 line-clamp-3 whitespace-pre-wrap text-xs text-gray-500 dark:text-muted-foreground">
                  {item.note}
                </div>
              ) : null}
            </button>
